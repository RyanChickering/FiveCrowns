...
class HandGraph:
    ...
    # Currently finding shortest paths, kind of want to find all paths or see how many sets you can have within
    # a hand. Pretty much still working through the same problem of things being usable in multiple sets at the
    # same time and what to do with that.
    # Distribute wildcards at the very end to check if you can go out.
    # Start with a list of all the available cards. Go through that list and select the group with the lowest loose
    # points
    # Explore all paths of a graph
    # Start at a node, go through nodes until you hit an ending. Probably just add final paths to a structure instead
    # of returning them.
    # Generate every path and then select with the ones that save the most points.
    def find_groups(self, curr_node, visited, path=Path()):
        path.nodes.append(curr_node)
        visited.append(curr_node)
        if len(path.nodes) >= 3 and path.cost <= 0:
            return path
        ...
        for edge in neighbors:
            ...
            if not visited.__contains__(edge[NODE]):
                # Check if the node is a wildcard. If it is, add it to wildcards
                if edge[NODE].name[SUIT_IDX] is 'J' or edge[NODE].name[VAL_IDX] is len(self.hand):
                    wildcards.append(edge)
                # If the card is not wild, check if it is adjacent to the current node
        ...
        while len(possibles) > 0:
            ...
            if path.type is NO_TYPE:
                if curr_edge[COST_VAL] < -1:
                    ...
                    path.type = RUN
            path.cost += curr_edge[COST_VAL]
            new_path = self.find_groups(curr_edge[NODE], visited, path)
            if len(new_path.nodes) > len(path.nodes):
                path = new_path
        return path
    ...
    def all_groups(self, hand):
        ...
        groups = []
        paths = []
        # Currently creates one path for each card in the hand. Want to create a sets limited to the cards in the hand
        for node in self.nodes:
